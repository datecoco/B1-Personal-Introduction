...
| 🔎 언어 필터 | `filter()`로 선택한 언어의 저장소 카드만 표시합니다. |
...
- `filter()`를 이용한 언어별 GitHub 저장소 카드 선택

### Flexbox와 Grid를 선택한 이유

- 메뉴는 한 줄에서 항목을 정렬하고 화면 크기에 따라 줄바꿈하면 되므로 **Flexbox**를 사용했습니다.
- 프로젝트 카드는 여러 행과 열로 배치하고 화면 크기에 따라 열 수를 바꿔야 하므로 **CSS Grid**를 사용했습니다.
